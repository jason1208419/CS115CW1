...
public class BouncingShapesWindow extends JFrame implements ActionListener
{
	...
	public void moveShapes()
	{
        ...
        /**
         * Method to flash the shapes present in the scene according to the specified colour.
         */
	public void flashColour()
	{
		//The colour showing on the screen now
		Color lastColour;
		//Go though each shape in the arrayList
		for (ClosedShape s : activeShapes)
		{
			lastColour = s.getColour();
			
			//exchange colour if the shape set to flash
			if (s.isFlash() && (currentTime % ANIMATION_DELAY == 0))
			{
				s.setColour(s.getFlashingColour());
				s.setFlashingColour(lastColour);
			}
